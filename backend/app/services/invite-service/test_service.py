...
class TestInviteService:
    ...
    async def test_create_invite_success(self, invite_service, mock_db, mock_email_service, sample_invite_create):
        ...
        assert len(result.code) > 0  # Verify code is not empty
        assert result.expires_at is not None
        # Verify expiration is approximately 7 days from now
        expected_expiry = datetime.now(timezone.utc) + timedelta(days=7)
        assert abs((result.expires_at - expected_expiry).total_seconds()) < 60  # Within 1 minute tolerance
    ...
    async def test_accept_invite_expired(self, invite_service, mock_db):
        ...
        mock_db.invites.update_one.assert_not_called()
    ...
    async def test_cleanup_expired_invites_success(self, invite_service, mock_db):
        ...
        # Verify the filter includes proper conditions for expired invites
        call_args = mock_db.invites.update_many.call_args
        filter_arg = call_args[0][0]
        assert "expires_at" in filter_arg
    ...
    async def test_resend_invite_email_success(self, invite_service, mock_db, mock_email_service):
        ...
        email_call_args = mock_email_service.send_organization_invite.call_args
        assert email_call_args[1]["to_email"] == "user@example.com"
        assert email_call_args[1]["invite_code"] == "resend_code"
        assert email_call_args[1]["organization_name"] == "Test Org"
